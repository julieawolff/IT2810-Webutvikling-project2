describe("Favoriting functionality", () => {
  ...
  it("checks that it works to favorite and unfavorite on main page", () => {
    // Favoriting A1 and Ace
    cy.contains(".cocktail-card", "A1").within(() => {
      cy.get('[data-testid="FavoriteBorderIcon"]').should("exist").click();
    });
    cy.contains(".cocktail-card", "Ace").within(() => {
      cy.get('[data-testid="FavoriteBorderIcon"]').should("exist").click();
    });
    ...
    cy.contains(".mini-cocktail-card", "A1").within(() => {
    ...
    cy.contains(".mini-cocktail-card", "Ace").within(() => {
      cy.get('[data-testid="FavoriteIcon"]').should("exist").click();
      cy.get('[data-testid="FavoriteBorderIcon"]').should("exist");
    });
  });

  it("checks that it works to favorite on cocktail page", () => {
  ...
  it("should return the cocktail of clicked card in favorites overview", () => {
    cy.contains("A1").click();
    cy.get('[data-testid="FavoriteBorderIcon"]').should("exist").click();
    cy.get('[data-testid="FavoriteIcon"]').should("exist");
    cy.visit("/user/Testinge2e");
  ...
  it("should show the error message when user has no favorites", () => {
    cy.visit("/user/Testinge2e");
